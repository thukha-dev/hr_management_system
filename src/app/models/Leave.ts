...
const LeaveSchema: Schema = new Schema<ILeave>(
  {
    ...
    halfDayTime: {
      ...
      required: function(this: ILeave) {
        return this.leaveType === LeaveType.HALF_UNPAID || this.leaveType === LeaveType.UNPAID;
      },
    },
    reason: {
      type: String,
...
LeaveSchema.virtual('duration').get(function(this: ILeave) {
  return (this.leaveType === LeaveType.HALF_UNPAID || this.leaveType === LeaveType.UNPAID) ? 0.5 : 1;
});

// Pre-save hook to calculate days
LeaveSchema.pre<ILeave>('save', function(next) {
  this.days = (this.leaveType === LeaveType.HALF_UNPAID || this.leaveType === LeaveType.UNPAID) ? 0.5 : 1;
  next();
});
