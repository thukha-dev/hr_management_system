...
type LeaveType = 'annual' | 'sick' | 'unpaid' | 'maternity' | 'paternity' | 'half-unpaid';
...
export default function LeavePage() {
  ...
  const handleSubmitLeaveRequest = (values: any) => {
    ...
    toast.success(`Your ${values.leaveType} leave request has been submitted for approval.`);
  ...
  return (
    <div className="container mx-auto p-3 sm:p-4 space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <Button onClick={() => setIsDialogOpen(true)} className="w-full sm:w-auto mt-2 sm:mt-0">
          ...
          <TabsContent value="my-leave" className="p-3 sm:p-4">
            <div className="space-y-3 sm:space-y-4">
              {leaveRequests.map((request) => (
                <div key={request.id} className="border rounded-lg p-3 sm:p-4 hover:bg-accent/50 transition-colors">
